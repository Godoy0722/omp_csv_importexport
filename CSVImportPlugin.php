...
use APP\plugins\importexport\csv\classes\commands\MonographCommand;
...
{
    /** @var string Command being used from CLI (supports "monographs" or "users") */
    ...
    private function displayImportForm(PKPRequest $request): void
    {
        ...
        $templateMgr->assign('csvImportPluginConfig', json_encode([
            ...
            'labels' => [
                ...
                'introIssues' => __('plugins.importexport.csv.results.intro.monographs'),
                'introIssuesDryMode' => __('plugins.importexport.csv.results.intro.monographs.dryMode'),
    ...
    private function handleImport(PKPRequest $request): void
    {
        ...
        if (!in_array($importType, ['monographs', 'users'])) {
    ...
    public function executeCLI($scriptName, &$args): void
    {
        ...
        if (! in_array($this->command, ['monographs', 'users']) || !$this->sourceDir || !$this->username) {
        ...
        $result = match ($this->command) {
            'monographs' => (new MonographCommand($this->sourceDir, $this->user, $dryMode))->run(),
